...
public class CardDisplay : MonoBehaviour
{
    ...
    private Vector3 originalPostition;              //드래그 전 원본 위치
    ...
    public void SetupCard(CardData data)
    {
        ...
        //카드 설명 텍스트웨 추가 효과 설명 추가
        if (descriptionText != null)
        {
            descriptionText.text = data.description + data.GetAdditionalEffectDesrition();
        }
    }
    ...
    private void OnMouseUp()
    {
        ...
        {
            CharacterState enemyState = hit.collider.GetComponent<CharacterState>();        //적에게 공격 효과 적용

            if(enemyState != null)
            {
                if (cardData.cardType == CardData.CardType.Attack)
                {
                    enemyState.TakeDamage(cardData.effectAmount);
            ...
            {
                Debug.Log("이 카드는 적 에게 사용 할 수 없습니다");
            }
        }
            ...
            if (CardManager.Instance.playerStats != null)
            {
                if (cardData.cardType == CardData.CardType.Heal)
                {
                    ...
                    Debug.Log($"{cardData.cardName} 카드로 플레이어 체력을 {cardData.effectAmount} 회복했습니다");
                    cardUsed = true;
                }
            }
            else
        ...
        foreach (var effect in cardDataCopy.additionalEffects)
        {
            ...
            { 
                ...
                case CardData.AdditionalEffectType.DiscardCard:             //카드 버리기 구현(랜덤 버리기)
                     for(int i =0; i<effect.effectAmount; i++)
                    {
                        if (CardManager.Instance != null && CardManager.Instance.handCads.Count > 0)
                        {
                            int randomIndex = Random.Range(0, CardManager.Instance.handCads.Count); //손패 크기 기준으로 랜덤 인덱스 생성

                            Debug.Log($"랜덤 카드 버리기 : 선택된 인덱스 {randomIndex} , 현재 손패 크기 {CardManager.Instance.handCads.Count}");

                            if (cardIndexCopy < CardManager.Instance.handCads.Count)
                            {
                                if (randomIndex != cardIndexCopy)
                                {
                                    CardManager.Instance.DiscardCard(randomIndex);

                                    //만약 버린 카드의 인덱스가 현재 카드의 인덱스보다 작다면 현재카드의 인덱스를 1감소 시켜야함.
                                    if (randomIndex < cardIndexCopy)
                                    {
                                        cardIndexCopy--;
                                    }
                                ...
                                else if (CardManager.Instance.handCads.Count > 1)
                                {
                                    //다음 카드 선택
                                    int newIndex = (randomIndex + i) % CardManager.Instance.handCads.Count;
                            ...
                            {
                                //cardindexcopy가 더이상 유효하지 않을 경우 아무카드나 버림
                                CardManager.Instance.DiscardCard(randomIndex);
                            }
                        }
                    }
