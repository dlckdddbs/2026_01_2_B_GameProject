...
public class TextTarget : MonoBehaviour
{
    ...
    private void ShowDamage(int amout, bool isCritical)
    {
        if (DamageEffectManager.instance != null)
        {
            Vector3 position = transform.position;
            position += new Vector3(Random.Range(-0.5f, 0.5f), Random.Range(1.0f, 1.5f), 0);
            DamageEffectManager.instance.ShowDamager(position,amout,isCritical);
        }
    }

    private void ShowHeal(int amout, bool isCritical)
    {
        if(DamageEffectManager.instance!= null)
        {
            Vector3 position = transform.position;
            position += new Vector3(Random.Range(-0.5f, 0.5f), Random.Range(1.0f, 1.5f), 0);
            DamageEffectManager.instance.ShowHeal(position,amout,isCritical);
        }
    }
    ...
    {
        float randomValue = Random.value;           //랜덤값으로 결정
